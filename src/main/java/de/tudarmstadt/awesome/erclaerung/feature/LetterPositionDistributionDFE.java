...
public class LetterPositionDistributionDFE extends FeatureExtractorResource_ImplBase {
	public static final String FN_LETTER_POSITION_PREFIX = "Letter_Position";
		...
		for (String token : tokens) {
			...
			for (int i = 0; i < letterArray.length; i++) {
				...
				lettersAndPositions.put(letterArray[i].toString() + i,
				                lettersAndPositions.get(letterArray[i].toString() + i));
			}
		}
