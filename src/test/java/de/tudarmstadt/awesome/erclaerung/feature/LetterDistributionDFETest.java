...
public class LetterDistributionDFETest {
	...
	public void extractTest1() throws Exception {
		LetterDistributionDFE extractor = new LetterDistributionDFE();
		List<Feature> features = extractor.extract(this.jcas1);

		assertEquals(30, features.size());

		for (Feature feature : features) {
			if (feature.getName().equals("LetterRatio_A"))
				FeatureTestUtil.assertFeature("LetterRatio_A", 52d / 742d, feature, 0.0001);
			if (feature.getName().equals("LetterRatio_C"))
				FeatureTestUtil.assertFeature("LetterRatio_C", 21d / 742d, feature, 0.0001);
			if (feature.getName().equals("LetterRatio_Y"))
				FeatureTestUtil.assertFeature("LetterRatio_Y", 7d / 742d, feature, 0.0001);
		}
	}
}
